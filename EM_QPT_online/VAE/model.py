...
#             nn.Conv2d(2, 32, kernel_size=3, stride=1, padding=1),  # (batch, 32, 4, 4)
#             nn.ReLU(),
#             nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),  # (batch, 64, 4, 4)
#             nn.ReLU(),
#             nn.BatchNorm2d(64),
#             nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),  # (batch, 128, 4, 4)
#             nn.ReLU(),
#             nn.Flatten(),
#         )
...
#             nn.ConvTranspose2d(128, 64, kernel_size=3, stride=1, padding=1),  # (batch, 64, 4, 4)
#             nn.ReLU(),
#             nn.BatchNorm2d(64),
#             nn.ConvTranspose2d(64, 32, kernel_size=3, stride=1, padding=1),   # (batch, 32, 4, 4)
#             nn.ReLU(),
#             nn.ConvTranspose2d(32, 2, kernel_size=3, stride=1, padding=1),    # (batch, 2, 4, 4)
#         )
...
#     def decode(self, z):
#         h = self.fc_decoder(z).view(-1, 128, 4, 4)
#         img = self.decoder(h)
#         img1 = self.clean_cholesky_layer(img)
#         img2 = self.density_matrix_layer(img1)
#         return img2
...
class VAE(nn.Module):
    def __init__(self, latent_dim=2,input=16):
        ...
        # Encoder 网络
        self.encoder = nn.Sequential(
            nn.Conv2d(2, 16, kernel_size=3, stride=1, padding=1),  # (batch, 16, 4, 4)
            #nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1),  # (batch, 32, 4, 4)
            #nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.Flatten(),  # 展平
        )
        ...
        # 计算均值和方差
        self.fc_mu = nn.Linear(32 * 4 * 4, latent_dim)  # 输出均值
        self.fc_logvar = nn.Linear(32 * 4 * 4, latent_dim)  # 输出 log 方差

        # Decoder 网络
        self.fc_decoder = nn.Linear(latent_dim, 32 * 4 * 4)  # 从潜在空间恢复
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(32, 16, kernel_size=3, stride=1, padding=1),  # (batch, 16, 4, 4)
            #nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.ConvTranspose2d(16, 2, kernel_size=3, stride=1, padding=1),  # (batch, 2, 4, 4)
        )
    ...
    def decode(self, z):
        h = self.fc_decoder(z).view(-1, 32, 4, 4)
        img = self.decoder(h)
        #print(img.shape)
        img1 = self.clean_cholesky_layer(img)
        #print(img1.shape)
