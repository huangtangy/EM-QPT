...
if __name__ == "__main__":
    ...
    # num_data = np.load('./trainingdata/fig_id_CZ_exp_100.npz')['chi_list']
    # num_data = transfer_to_imag(num_data)  # for experimental data
    num_data = np.load('./trainingdata/fig_id_005p_2q_1024_27Feb.npz')['fig_id_gan']  # 'fig_id_gan_01p_1q_1024.npz'

    #np.load('fig_id_001p_2q_1024.npz')['fig_id_gan']#'fig_id_gan_01p_1q_1024.npz'
    ...
    #=========model evaluation=======
    torch.save(model.state_dict(), './saved_model/2q/model_weights_2q'+str(trainnum)+'_p='+str(p)+'.pth')
    model = VAE(latent_dim,input)
    model.load_state_dict(torch.load('./saved_model/2q/model_weights_2q'+str(trainnum)+'_p='+str(p)+'.pth'))
    model.eval()
    with torch.no_grad():
        z = torch.randn(400, latent_dim)#.to(device)  # 随机采样 latent vector
        sample0 = model.decode(z)
        sample1 = clean_cholesky(sample0.reshape(400,2,16,16))
        ...
        print('is it CPTP?',is_CPTP_chi_np(sample2_numpy[0],2 ))
    ...
    images = num_data[200:300, :, :]
    fig, axes = plt.subplots(2, 10, figsize=(10, 2))
    for i in range(10):
        recons_data = sample[i][0]#sample[i]
        ...
        axes[0, i].axis("off")
        axes[1, i].axis("off")
    plt.suptitle("Top: Original | Bottom: Reconstructed (first channel)")

    #plt.savefig("fig_cz_diggital_im.pdf", format="pdf", dpi=600, bbox_inches='tight')
    plt.show()

    fig, axes = plt.subplots(2, 10, figsize=(10, 2))
    for i in range(10):
        #recons_data = sample[i]
        recons_data = sample[i][1]
        #recons_data = reconstruct_images(recons_data)
        axes[0, i].imshow(num_data1[i, 1], cmap="gray")  # 原始输入
        axes[1, i].imshow(recons_data, cmap="gray")  # 生成输出
        axes[0, i].axis("off")
        axes[1, i].axis("off")
    plt.suptitle("Top: Original | Bottom: Reconstructed (second channel)")

    #plt.savefig("fig_cz_diggital_re.pdf", format="pdf", dpi=600, bbox_inches='tight')
    plt.show()